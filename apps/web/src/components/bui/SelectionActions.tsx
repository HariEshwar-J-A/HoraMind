// @ts-nocheck — vendored third-party source, deliberately unchecked.
//
// This project sets noUncheckedIndexedAccess, which Beautiful UI does not,
// so every arr[i] here reads as possibly-undefined and the file will not
// compile untouched. The alternative is editing nineteen upstream files to
// satisfy a flag their author never opted into, which makes the next update
// a merge conflict instead of a copy. Our own code stays fully checked; only
// what we copied verbatim is exempt.
/*
 * Vendored from Beautiful UI — https://www.beautifului.dev/
 * Source: components/SelectionActions.tsx. Unmodified except for removing the
 * Next-only 'use client' directive and repointing the two unpublished
 * atoms at ./atoms.tsx, which reimplements their props exactly.
 *
 * Do not restyle this file. Every colour here is a semantic token, so the
 * palette is set once in beautiful-ui.css and inherited; editing classes
 * locally is what makes a vendored component impossible to update later.
 */
...
import { Shimmer } from "./atoms.js";
import { StreamText } from "./atoms.js";
